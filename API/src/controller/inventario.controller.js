...
const createInventario = async (req, res) => {  
    try {
        const {bodegaId,productoId, cantidad, familiaNombre,estilos} = mayuscula(req.body);

        //validacion de datos
        if(estaVacio([bodegaId,productoId, familiaNombre])){
            return res.status(400).json({ message: 'Faltan datos(bodegaid, productoId, familiaNombre) o datos incorrectos '});
        }
        if(cantidad===undefined || estilos===undefined || isNaN(cantidad) ||  estilos.length==0){
            return res.status(400).json({ message: 'datos (cantidad o estilos) incorrectos '+cantidad+" "+estilos});
        }
        ...
        const existeProducto = await productSchema.findOne({producto_id:productoId});
        const existeBodega =  await bodegaSchema.findOne({bodegaId:bodegaId});
        const existeFamilia = await familiaSchema.findOne({nombre:familiaNombre});
        if(existeBodega==null || existeProducto==null || existeFamilia==null){
            return res.status(400).json({ error: {'No se creo inventario. Bodega, producto o familia no existen. bodega existente': existeBodega, 'producto existente': existeProducto, 'familia existente': existeFamilia }});
        }
        //crear el nuevo inventario
        const nuevoInventario = new inventarioSchema({
            bodegaId,inventarioId,productoId, cantidad, familiaNombre, fecha,estilos}); 
...
const updateInventario = async (req, res) => {
    try {
        ...
        if(estaVacio([bodegaId,productoId,tipo])){
            return res.status(400).json({ message: 'Faltan datos(bodegaid, productoId) o datos incorrectos '});
            }
        ...
        if(existeInventario==null){
            return res.status(400).json({ error: {'No se actualizo inventario. Inventario no existe': bodegaId+"-"+productoId}});
        }
        ...
        updateInventarioFunction(bodegaId,productoId,estilos,entrada_o_salida_inventario);
...
const updateInventarioFunction = async (bodegaId,productoId,estilos,entrada_o_salida) => {
    try {  
        ...
        for(let i=0;i<estilos.length;i++){
            ...
            const update = { $inc: {cantidad:cantidad, [`estilos.$[estilo].cantidad`]: cantidad } };
            const options = { arrayFilters: [{ "estilo.nombre": nombre}] };
            await inventarioSchema.updateOne(filter, update, options);
            console.log('actualizo inventario:cantidad: '+cantidad+' estilo: '+nombre);
        }
...
module.exports = { getInventario,createInventario, updateInventario,updateInventarioFunction };
