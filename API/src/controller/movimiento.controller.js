...
const createMovimiento = async (req, res) => {
  try {
    const { cantidad, categoria, productoId, bodegaId, tipo, estilos } =
      mayuscula(req.body);
    ...
    if (estaVacio([categoria, productoId, bodegaId, tipo])) {
      return res
        ...
        .json({
          message:
            "Faltan datos(categoria, productoId, bodegaId,tipo) o datos incorrectos ",
        });
    }
    if (
      isNaN(cantidad) ||
      ...
      return res
        ...
        .json({
          message:
            "La cantidad debe ser un numero y estilos no debe estar vacio",
        });
    }
    //existe inventario
    ...
    if (existeInventario == null) {
      return res
        ...
        .json({
          error: {
            "No se hizo moviminiento ni se actualizo inventario. Inventario no existe":
              bodegaId + "-" + productoId,
          },
        });
    }
    ...
    if (!movimientosCreados) {
      return res
        .status(400)
        .json({ message: "No se pudo crear el movimiento" });
    }
...
const createMovEntreBodegas = async (req, res) => {
  try {
    ...
    const { productoId, bodegaId1, bodegaId2, cantidad, estilos } = mayuscula(
      req.body
    );
    ...
    if (estaVacio([productoId, bodegaId1, bodegaId2])) {
      return res
        ...
        .json({
          message:
            "Faltan datos( productoId, bodegaId) o datos incorrectos " +
            productoId +
            "-" +
            bodegaId1 +
            "-" +
    ...
    //existe inventario
    const existeInventario1 = await inventarioSchema.findOne({
      inventarioId: bodegaId1 + "-" + productoId,
    });
    const existeInventario2 = await inventarioSchema.findOne({
      inventarioId: bodegaId2 + "-" + productoId,
    });
    ...
    if (!movimientosCreados1) {
      return res
        ...
        .json({
          message:
            "No se pudo crear el movimiento de ingreso a la bodega" + bodegaId1,
        });
    }
    ...
    if (!movimientosCreados2) {
      return res
        ...
        .json({
          message:
            "No se pudo crear el movimiento de salida de la bodega " +
            bodegaId2,
        });
    }
    ...
    res
      .status(500)
      .json({ error: "No se pudo crear el movimiento: " + err.message });
  }
};
  ...
  try {
    ...
    if (
      ...
      return res
        ...
        .json({
          message:
            "No se pudo eliminar el movimiento,revise que el _id sea valido y este activo",
        });
    }
...
const createMovimientoFunction = async (datos) => {
  ...
  try {
    ...
    const newMovimiento = new movimientoSchema({
      tipo,
      fecha,
      cantidad,
      productoId,
    ...
    const mov_productos_inventario = await updateInventarioFunction(
      bodegaId,
      productoId,
      estilos,
    ...
    console.log(
      "check point 2:",
      datosMayusculas,
      "mov_procustos_inventarios: ",
      mov_productos_inventario,
      "dentro de la funcion createMovimientoFunction en movimientoControlador",
      "estilo:",
      estilos,
      "bodegaid:",
      bodegaId,
    ...
    console.log(
      "checpoint3 : despues de mov inventarios)",
      "bodegaId ",
      bodegaId
    );
    //guardar movimiento
...
const deleteMovimientoFunction = async (idMovimiento, usuario) => {
  ...
  try {
    ...
    const fechaEliminacion = moment()
      .tz("America/Bogota")
      .format("YYYY-MM-DD HH:mm:ss");

    //buscando documento
    const movimiento = await movimientoSchema.findById({
      _id: idMovimiento,
      activo: true,
    });
    ...
    console.log(
      "check point 1 functiondeleteMov:",
      estilos,
      cantidad,
      tipo,
      bodegaId,
